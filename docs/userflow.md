...
3.  **출력 (Output)**
    ...
    *   **[Edge Case Feedback]** 인증 실패 시, 사용자에게 로그인이 필요하다는 안내와 함께 로그인 모달을 표시할 수 있습니다.
...
1.  **입력 (Input)**
    *   사용자가 로그인 필요 기능(찜하기, 헤더 로그인 버튼 등)을 시도합니다.
    *   사용자가 로그인 모달에 이메일 및 비밀번호를 입력하고 '로그인' 버튼을 클릭합니다.
...
3.  **출력 (Output)**
    *   **인증 성공 시:** 로그인 모달이 닫히고, 헤더 UI가 변경되며, 로그인을 요청했던 기능/페이지가 실행됩니다.
    *   **인증 실패 시:** 모달 내부에 실패 원인(예: "비밀번호가 일치하지 않습니다")을 표시합니다.
    *   **[Edge Case Feedback]** 클라이언트 측 유효성 검증 실패 시, 해당 입력 필드에 안내 문구를 표시합니다.
    *   **[Edge Case Feedback]** 서버 통신 오류 시, 모달 내부에 오류 발생 메시지를 표시합니다.
...
2.  **처리 (Processing)**
    ...
    *   **2-4. [Edge Case]** 로그인이 풀린 상태로 접근 시 접근을 차단하고 로그인 UI를 표시합니다.
...
3.  **출력 (Output)**
    ...
    *   **[Edge Case Feedback]** 로그인이 필요한 경우, "로그인이 필요한 서비스입니다" 안내와 함께 로그인 UI를 표시합니다.